...
assert.equal(res.status, 404, "debug proxy route is gone");
res = await worker.fetch(new Request("https://x/api/debug-card?mode=search&name=Charizard", { headers: { "X-Admin-Token": "admin-secret" } }), env, ctx);
assert.equal(res.status, 404, "debug proxy route is gone even for admins");
