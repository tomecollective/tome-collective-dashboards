...
async function buildDashboard(env, { date, mode } = {}) {
  ...
  const playerBase = playerIds.map((pid) => {
    ...
    for (const obj of objectives) {
      ...
      // overwrite those with an L10 passthrough). PITP is the one confirmed
      // exception -- neither Rotowire nor standard WNBA stats sites project
      // it, so it always falls back to the L10 average. Every other stat with
      // no uploaded override is left as an explicit TBD (null) rather than
      // silently defaulting to L10 -- the admin hasn't sourced real numbers
      // for this date/stat yet, so nothing here should look authoritative.
  ...
  const baseNote =
    "Live BALLDONTLIE data. YTD/L10 always reflect current form (as of today), regardless of which run date is selected. PITP (when an objective) is derived from player_game_advanced_stats.stats.misc.points_paint and always uses the L10 average as Proj (Rotowire doesn't project it). Every other objective shows Proj as TBD until the admin uploads that date's Rotowire projections for it -- it never silently falls back to L10. Weight is auto-computed from the share of players projected to clear 100% of target -- the rarer objective carries more weight.";
